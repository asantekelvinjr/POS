...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Redirect based on role — the store has the user after login
      const stored = localStorage.getItem('pos-auth')
      const role = stored ? JSON.parse(stored)?.state?.user?.role : null
  ...
  return (
    <div className="min-h-screen bg-base flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 flex-col justify-between p-12 relative overflow-hidden"
        ...
        <div className="absolute inset-0 opacity-5" style={{ backgroundImage: `radial-gradient(circle at 25% 25%, var(--color-accent) 0%, transparent 50%), radial-gradient(circle at 75% 75%, var(--color-highlight) 0%, transparent 50%)` }}/>
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-pos-md flex items-center justify-center font-bold text-lg text-white" style={{ backgroundColor: 'var(--color-accent)', fontFamily: 'var(--font-display)' }}>P</div>
            <span className="text-xl font-bold tracking-tight text-white" style={{ fontFamily: 'var(--font-display)' }}>PayPoint POS</span>
          </div>
        </div>

        <div className="relative z-10 space-y-6">
          <h1 className="text-5xl font-bold text-white leading-tight" style={{ fontFamily: 'var(--font-display)' }}>
            Sell smarter,<br/>
          ...
          <div className="flex flex-wrap gap-2 pt-2">
            {['Mobile Money', 'Real-time Inventory', 'Smart Reports', 'Multi-role Access'].map(f => (
              <span key={f} className="text-xs font-medium px-3 py-1.5 rounded-full" style={{ backgroundColor: 'rgba(16,185,129,0.15)', color: 'var(--color-accent)', border: '1px solid rgba(16,185,129,0.2)' }}>
                {f}
              </span>
            ))}
          </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 lg:p-16">
        <div className="w-full max-w-md space-y-8">
          <div className="flex items-center gap-3 lg:hidden">
            <div className="w-9 h-9 rounded-pos-md flex items-center justify-center font-bold text-white" style={{ backgroundColor: 'var(--color-accent)', fontFamily: 'var(--font-display)' }}>P</div>
            <span className="text-lg font-bold" style={{ fontFamily: 'var(--font-display)' }}>PayPoint POS</span>
          </div>

          <div>
            <h2 className="text-3xl font-bold text-primary" style={{ fontFamily: 'var(--font-display)' }}>Welcome back</h2>
            <p className="mt-2 text-secondary">Sign in to your account to continue</p>
          </div>

          <form onSubmit={handleSubmit} className="space-y-5">
            <Input label="Email address" type="email" value={email} onChange={e => setEmail(e.target.value)}
              placeholder="cashier@store.com" required autoComplete="email"/>
            ...
            {error && (
              <div className="flex items-center gap-2.5 p-3.5 rounded-pos-md text-sm"
          ...
          <p className="text-center text-sm text-muted">
            Need an account?{' '}
            <span className="text-accent font-medium">Contact your administrator</span>
          </p>
        </div>
      </div>
    </div>
