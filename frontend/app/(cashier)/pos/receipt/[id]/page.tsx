...
interface SaleDetail {
  ...
  customer_name: string | null; total_amount: number; subtotal: number; tax_amount: number
  discount_amount: number; payment_method: string; status: string
  items: SaleItem[]
  payment: { paystack_ref: string | null; method: string; amount_tendered: number | null; change_given: number | null }
}
  ...
  return (
    <div className="max-w-md mx-auto space-y-6 py-8">
      ...
      <div className="text-center space-y-3">
        <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto" style={{ backgroundColor: 'var(--color-success-light)' }}>
        ...
        <div>
          <h1 className="text-2xl font-bold text-primary" style={{ fontFamily: 'var(--font-display)' }}>Payment Successful!</h1>
          <p className="text-secondary mt-1">Transaction completed via {sale.payment_method}</p>
        </div>
      </div>

      {/* Receipt card */}
      <div className="card overflow-hidden">
        <div className="px-6 py-5 text-center border-b border-base" style={{ backgroundColor: 'var(--color-accent)', color: 'white' }}>
          <h2 className="font-bold text-lg" style={{ fontFamily: 'var(--font-display)' }}>
            {process.env.NEXT_PUBLIC_STORE_NAME || 'PayPoint POS'}
          </h2>
          <p className="text-sm opacity-80 mt-0.5">{process.env.NEXT_PUBLIC_STORE_ADDRESS || 'Accra, Ghana'}</p>
        </div>
        ...
        <div className="px-6 py-4 space-y-1.5 text-sm">
          ...
          {sale.payment?.amount_tendered && (
            <div className="flex justify-between text-secondary">
              <span>Tendered</span><span>GHS {Number(sale.payment.amount_tendered).toFixed(2)}</span>
            </div>
          )}
