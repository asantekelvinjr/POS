...
export default function POSPage() {
  ...
  return (
    <div className="h-[calc(100vh-4rem)] flex flex-col lg:flex-row gap-0 overflow-hidden -m-6">
      ...
      {/* Right: Cart panel */}
      <div
        className="w-full lg:w-96 flex flex-col border-t lg:border-t-0 lg:border-l border-base"
      ...
      >
        ...
        <div className="px-5 py-4 border-b border-base flex items-center justify-between">
          ...
          {cartItems.length > 0 && (
            <button
              onClick={clearCart}
              className="text-xs text-danger hover:text-danger transition-pos font-medium"
            >
              Clear all
            </button>
          )}
        ...
        <div className="px-5 py-5 border-t border-base space-y-3">
          ...
          <button
            ...
            style={{
              backgroundColor: cartItems.length > 0 ? 'var(--color-accent)' : 'var(--color-surface-3)',
            }}
          >
          ...
          <div className="flex gap-2">
            {['Cash', 'MoMo', 'Card'].map(m => (
              <span
                key={m}
                className="flex-1 text-center text-xs py-1.5 rounded-pos-sm text-muted bg-surface-2 font-medium"
              >
                {m}
              </span>
            ))}
